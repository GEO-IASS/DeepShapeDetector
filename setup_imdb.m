...
target_size = 64;
label_size = 3;
%gen_and_save_shape(train_path, target_size, train_size);
gen_and_save_shape(val_path, target_size, val_size);
gen_and_save_shape(test_path, target_size, test_size);

total_sample_size = train_size + val_size + test_size;
imdb.meta.image_size = target_size;
...
%     end
    ...
    for s = 1:sizes(i)
        ...
        if i > 1
            ...
            % Organizing location
            imdb.images.labels(:, count) = pos_struct.all_pos_triangle(s, :)/target_size;
        end
